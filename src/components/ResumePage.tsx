...
import {
    ...
    ExperienceItem,
    HonorItem,
} from '../assets/resumeData';

type TimeLineItem = EducationItem | ExperienceItem | HonorItem;
type TimeLineTitle = 'Education' | 'Experience' | 'Honors';

interface EducationItemProps extends EducationItem {}
interface ExperienceItemProps extends ExperienceItem {}
interface HonorItemProps extends HonorItem {}

const EducationItemComp = ({ name, period, location, specifics }: EducationItemProps) => (
...
const ExperienceItemComp = ({ position, company, link, period, location, specifics }: ExperienceItemProps) => (
    <div id="timeline-item" className="mb-4 p-3 border-2 rounded-lg border-gray-500 w-full">
...
const HonorItemComp = ({ name, period, what }: HonorItemProps) => (
    <div id="timeline-item" className="mb-4 p-3 border-2 rounded-lg border-gray-500 w-full">
        <h3 className="text-l font-semibold">{name}</h3>
        <div className='flex flex-row gap-1'>
...
const TimeLineContainer = ({ list, title }: TimeLineContainerProps) => (
        ...
        <div className="border-l-2 border-black">
            {list.map((item, index) => (
                <div className="flex flex-row justify-items-start align-middle" key={index}>
                    ...
                    {title === "Education" && <EducationItemComp {...(item as EducationItem)} />}
                    {title === "Experience" && <ExperienceItemComp {...(item as ExperienceItem)} />}
                    {title === "Honors" && <HonorItemComp {...(item as HonorItem)} />}
                </div>
            ))}
        </div>
    </div>
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="flex flex-col py-5 items-center flex-grow bg-gray-100 mt-10">
                <div className={`flex ${isDesktop ? "flex-row" : "flex-col-reverse"} w-11/12 rounded-xl border-gray-600 border-solid border-2 mt-10`}>
...
const ResumeNavbar = ({ isTopOfPage }: ResumeNavbarProps) => {
        ...
        <nav className={`transition-all duration-200 z-40 w-full fixed top-0 py-6 text-white ${navbarBackground}`}>
            <div className='flex items-center justify-center gap-10 mx-auto px-16'>
                <div className='flex justify-between gap-16 font-opensans text-sm font-semibold'>
                    <Link to="/home" className={`relative inline-block cursor-pointer transition duration-500 ${defaultSize} group`}>
                        <div className='max-w-md'>
                            Home
                            <span className="absolute bottom-0 left-0 inline-block w-full h-0.5 bg-white transform scale-x-0 origin-left transition-transform duration-300 group-hover:scale-x-100" />
                        </div>
                    </Link>
                </div>
                <div className='flex justify-between gap-16 font-opensans text-sm font-semibold'>
